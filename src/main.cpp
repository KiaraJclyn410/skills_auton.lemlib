...
lemlib::ControllerSettings angular_controller(7.5, // proportional gain (kP)
                                              0.25, // integral gain (kI)
                                              70.5, // derivative gain (kD)
                                              2, // anti windup
...
void initialize() {
    ...
    while (true) {
		...
		double leftTemp = left_motors.get_temperature(); 
...
void autonomous() {
    ...
    chassis.moveToPoint(47.5, -47, 1000);
    ...
    backspin_outtake();
	//to matchloader
	chassis.moveToPoint(50, -57.55, 1000);
	pros::delay(3000);
    ...
    stop_intake();
	//long move
	chassis.moveToPoint(60, 40, 7000, {.forwards = false});
	...
	//back into goal
	chassis.moveToPose(chassis.getPose().x, 23.4, 0, 1000, {.forwards = false});
    start_intake();
    backspin_intake();
    start_intake();
    pros::delay(1000);
	...
	chassis.setPose(47, 31, (imu.get_heading() + 89));
    stop_outtake();
    stop_intake();
	//go to matchloader
	chassis.moveToPoint(47, 60.9, 7000);
    start_intake();
	pros::delay(2500);
	//back into goal
	chassis.moveToPose(46.5, 23.4, 0, 7000, {.forwards = false});
    start_intake();
    backspin_intake();
    pros::delay(200);
    start_intake();
    ...
    stop_intake();
	//pose reset on goal
	chassis.setPose(48, 31, (imu.get_heading() + 89));
    stop_outtake();
    stop_intake();
    hood_pneu.set_value(false);
